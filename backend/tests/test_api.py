...
def test_ask_persists_turn_with_trace(client):
    r = client.post("/api/chat/ask", json={"question": "Does Koh curve?"})
    assert r.status_code == 200
    body = r.json()
    assert body["intent"] == "professor_info" and body["mode"] == "extractive"
    assert body["citations"] and body["citations"][0]["n"] == 1

    detail = client.get(f"/api/history/{body['conversation_id']}").json()
    assert [m["role"] for m in detail["messages"]] == ["user", "assistant"]
    assert detail["messages"][1]["intent"] == "professor_info"


def test_followup_uses_history(client, fake_llm):
    first = client.post("/api/chat/ask", json={"question": "Tell me about Jill Seaman"}).json()
    seen = {}

    def router(messages):
        seen["prompt"] = messages[1]["content"]
        return {"intent": "professor_info", "standalone_question": "Does Jill Seaman curve?",
                "professors": ["Jill Seaman"], "courses": [], "completed_courses": []}

    fake_llm(router_fn=router, answer="Reviewers mention quizzes [1].")
    r = client.post("/api/chat/ask", json={"question": "does she curve?",
                                           "conversation_id": first["conversation_id"]})
    assert r.status_code == 200
    assert "Tell me about Jill Seaman" in seen["prompt"]


def test_stream_event_sequence(client, fake_llm):
    fake_llm(answer="Seaman's reviews praise clear lectures [1].")
    r = client.post("/api/chat/ask/stream", json={"question": "Is Jill Seaman good for CS2308?"})
    assert r.status_code == 200
...
def test_answer_cache_serves_repeat_question(client, fake_llm):
    ...
    client.post("/api/chat/ask", json={"question": "Does Guirguis curve?"})
    synth_calls = fake.calls.count("synth")
    r = client.post("/api/chat/ask", json={"question": "does guirguis curve"})
    assert r.status_code == 200 and fake.calls.count("synth") == synth_calls


def test_feedback_and_agent_analytics(client):
    body = client.post("/api/chat/ask", json={"question": "Is Qasem good?"}).json()
